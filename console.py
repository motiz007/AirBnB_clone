...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, arg):
        ...
        if tokens[0] not in HBNBCommand.my_classes.keys():
            print("** class doesn't exist **")
            return
        new_object = HBNBCommand.my_classes[tokens[0]]()
        new_object.save()
        print(new_object.id)
    ...
    def do_all(self, arg):
        ...
        if token[0] in HBNBCommand.my_classes.keys():
            for key in objects_dict:
                if token[0] in key:
                    my_json.append(str(objects_dict[key]))
            print(json.dumps(my_json))
    ...
    def do_update(self, arg):
        ...
        storage.reload()
        objects_dict = storage.all()
        if my_data[0] not in HBNBCommand.my_classes.keys():
            print("** class doesn't exist **")
            return
        if (len(my_data) == 1):
            print("** instance id missing **")
    ...
    def do_update2(self, arg):
        ...
        my_classesionary = my_classesionary.replace("\'", "\"")
        my_classesionary = json.loads(my_classesionary)
        my_instance = objects_dict[key]
        for my_key in my_classesionary:
            if hasattr(my_instance, my_key):
                data_type = type(getattr(my_instance, my_key))
                setattr(my_instance, my_key, my_classesionary[my_key])
            else:
                setattr(my_instance, my_key, my_classesionary[my_key])
        storage.save()
